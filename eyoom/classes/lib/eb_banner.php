		...
		foreach ($banners as $item) {
			...
			if ($item['bn_type'] == 'intra') {
				if ($item['bn_link'] != '' && $item['bn_link'] != 'nolink') {
					...
					if ($bn_no == 7) {
						$result = $prefix.'<a id="banner_'.$bn_no.'" href="'.G5_BBS_URL.'/banner.php?tocken='.$tocken.'" target="'.$item['bn_target'].'">'.$item['tag_img'].'</a><div style="clear:both;display:block;overflow:hidden"><a style="display:block;float:left;width:50%" href="https://www.instagram.com/tombilli_official" target="_blank"><img src="https://i.imgur.com/kzo0Z1S.png" style="max-width:100%" alt="인스타그램에서 톰빌리 만나기"></a><a style="display:block;float:left;width:50%" href="'.G5_BBS_URL.'/banner.php?tocken='.$tocken.'" target="'.$item['bn_target'].'"><img src="https://i.imgur.com/lOKdHKd.png" style="max-width:100%" alt="톰빌리 수여니의 영상 메시지 보기"></a></div>'.$sufix;
					} else {
