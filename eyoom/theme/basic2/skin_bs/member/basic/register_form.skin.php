...
<div class="register-form">
	...
	<script src="/eyoom/theme/basic2/js/zxcvbn.js"></script>
	<script src="/eyoom/theme/basic2/js/jquery.password.strength.js"></script>
	...
	<div class="register-box">
		<header><h5><strong>사이트 이용정보 입력</strong></h5></header>
		<fieldset>
			<div class="row">
				<section class="col col-4">
					<label for="reg_mb_id" class="label width-100">
						아이디<strong class="sound_only"> 필수</strong>
						<?php if ($w == '') { ?>
						<span class="pull-right">
							<a href="javascript:;" class="btn-e btn-e-dark btn-e-sm" type="button" onclick="check_duplication('mb_id');"><span class="color-white">중복체크</span></a>
						</span>
					...
					<label class="input">
						...
						<input type="text" name="mb_id" value="<?php echo $member["mb_id"] ?>" id="reg_mb_id" <?php if ($w!='') { ?>required readonly<?php } ?> minlength="3" maxlength="20"><?php if ($w == '') { ?><input type="hidden" name="mb_id_duplicated" id="mb_id_duplicated"><?php } ?>
		...
		<header class="border-top"><h5><strong>개인정보 입력</strong></h5></header>
		<fieldset>
			<div class="row">
				<section>
			...
			<div class="col col-8">
  				...
  				<section>
  					<div class="col col-4">
  						<label for="reg_mb_nick" class="label width-100">
  							닉네임<strong class="sound_only">필수</strong>
  							<?php if ($w == '') { ?>
  							<span class="pull-right">
  								<a href="javascript:;" class="btn-e btn-e-dark btn-e-sm" type="button" onclick="check_duplication('mb_nick');"><span class="color-white">중복체크</span></a>
  							</span>
  							<div class="clearfix"></div>
  							<?php } ?>
  						</label>
			...
			<div class="row">
				<section>
					<div class="col col-4">
						<label for="reg_mb_email" class="label width-100">
							이메일<strong class="sound_only"> 필수</strong>
							<?php if ($w == '') { ?>
							<span class="pull-right">
								<a href="javascript:;" class="btn-e btn-e-dark btn-e-sm" type="button" onclick="check_duplication('mb_email');"><span class="color-white">중복체크</span></a>
							</span>
							<div class="clearfix"></div>
							<?php } ?>
						</label>
			...
			<div class="row">
				<section>
					...
					<div class="col col-3">
						<button type="button" onclick="win_zip('fregisterform', 'mb_zip', 'mb_addr1', 'mb_addr2', 'mb_addr3', 'mb_addr_jibeon');" class="btn-e btn-e-dark">주소 검색</button>
					</div>
		...
		<header class="border-top"><h5><strong>기타 개인설정</strong></h5></header>
		<fieldset>
			<div class="row">
				...
				<section class="col col-12">
					...
					<textarea name="mb_signature" id="reg_mb_signature" <?php if ($config["cf_req_signature"]) { ?>required<?php } ?>><?php echo $member["mb_signature"] ?></textarea>
					</label>
				</section>
				<?php } ?>
			</div>
			<div class="row">
				...
				<section class="col col-12">
					...
					<label class="textarea textarea-resizable">
					<textarea name="mb_profile" id="reg_mb_profile" <?php if ($config["cf_req_profile"]) { ?>required<?php } ?>><?php echo $member["mb_profile"] ?></textarea>
					</label>
				</section>
				<?php } ?>
			</div>
			...
			<div class="row">
				...
				<section class="col col-6">
					...
					<div class="note margin-bottom-10"><strong>Note:</strong> 정보공개는 수정후 <?php echo $config["cf_open_modify"] * 1?>일 이내, <?php echo $GLOBALS["open_day"] ?> 까지는 변경이 안됩니다.<br>이렇게 하는 이유는 잦은 정보공개 수정으로 인하여 쪽지를 보낸 후 받지 않는 경우를 막기 위해서 입니다.</div>
				</section>
				<?php } ?>
		...
		<footer>
			...
			<div class="pull-right">
				<button type="submit" value="<?php if ($w == '') { ?>회원가입<?php } else { ?>정보수정<?php } ?>" id="btn_submit" class="btn-e btn-e-lg btn-e-red" accesskey="s"><?php if ($w == '') { ?>회원가입<?php } else { ?>정보수정<?php } ?></button>
			</div>
			<div class="clearfix"></div>
		</footer>
	</div>
...
.register-form .eyoom-form header {padding:10px 20px;background:#fafafa}
.register-form .eyoom-form footer {padding:10px 20px}
...
.register-form .security_1 .progress-e .progress-bar {background:#f44455}
