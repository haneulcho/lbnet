		...
		<div id="c_<?php echo $item["comment_id"] ?>" class="view-comment-item<?php if ($item["is_lb_admin"]) { ?> admin<?php } ?><?php if ($item["is_cmt_best"]) { ?> cmt-best<?php } ?>">
			<div class="comment-item-body-pn">
				...
				<div class="comment-item-info lbdes" style="<?php if ($item["cmt_depth"] && !$item["is_cmt_best"]) { ?>padding-left:<?php echo $item["cmt_depth"] ?>px;<?php } ?>">
					...
					<!--{* 공지사항에는 댓글 못달게 *}-->
					<?php if ($is_comment_write && ($view["ca_name"] != '공지' || $is_admin)) { ?>
				...
				<?php if ($is_ip_view && $is_admin) { ?>
					<div class="comment-item-info lbdes margin-bottom-0"><span class="comment-ip">(<?php echo $item["lb_id"] ?> / <?php echo $item["lb_nickname"] ?> / <?php echo $item["ip"] ?>)</span></div>
				<?php } ?>
				...
				<div class="comment-btn">
					<?php if ($item["c_good_href"] || $item["c_nogood_href"]) { ?>
					<ul class="list-inline reply-list pull-right">
						<?php if ($item["c_good_href"]) { ?>
						<li class="margin-left-5"><a href="<?php echo $item["c_good_href"] ?>" id="goodcmt_button_<?php echo $item["comment_id"] ?>" class="goodcmt_button" type="button" title="추천"><i class="fa fa-thumbs-up"></i> <strong><?php if ($item["good"]) { ?><span class="wow"><?php echo $item["good"] ?></span><?php } else { ?><span>0</span><?php } ?></strong></a></li>
						<?php } ?>
						<?php if ($item["c_nogood_href"]) { ?>
						<li><a href="<?php echo $item["c_nogood_href"] ?>" id="nogoodcmt_button_<?php echo $item["comment_id"] ?>" class="nogoodcmt_button" type="button" title="비추천"><i class="fa fa-thumbs-down"></i> <strong><?php if ($item["nogood"]) { ?><span><?php echo $item["nogood"] ?></span><?php } else { ?><span>0</span><?php } ?></strong></a></li>
						<?php } ?>

						<!--{* 댓글 신고 블라인드 시작 *}-->
						<?php if ($eyoom_board["bo_use_yellow_card"] == '1' && $is_member) { ?>
							<?php if ($item["is_mine"]) { ?>
								<li id="cmt_yellow_card_li_<?php echo $item["comment_id"] ?>"><button class="lbbtn_light" onclick="javascript:alert('자신의 댓글은 신고할 수 없습니다!')"><i class="fa fa-exclamation-triangle"></i> <span><?php echo number_format($item["yc_count"]) ?></span></button></li>
							<?php } else { ?>
								<?php if (!$mb_ycard["mb_id"]) { ?>
									<li id="cmt_yellow_card_li_<?php echo $item["comment_id"] ?>"><button id="cmt_yellow_card_<?php echo $item["comment_id"] ?>" class="lbbtn_light cmt_yellow_card" data-toggle="modal" data-target=".yellowcard-modal" data-cmt-id="<?php echo $item["comment_id"] ?>"><i class="fa fa-exclamation-triangle"></i> <span><?php echo number_format($item["yc_count"]) ?></span></button></li>
									<?php } else { ?>
									<li id="cancel_cmt_yellow_card_li_<?php echo $item["comment_id"] ?>"><button id="cancel_cmt_yellow_card_<?php echo $item["comment_id"] ?>" class="lbbtn_light cancel_cmt_yellow_card" data-cmt-id="<?php echo $item["comment_id"] ?>"><i class="fa fa-exclamation-triangle"></i> <span><?php echo number_format($item["yc_count"]) ?></span></button></li>
								<?php } ?>
							<?php } ?>

							<?php if ($is_admin && $blind_direct) { ?>
								<?php if ($item["yc_blind"] != 'y') { ?>
								<li data-placement="top" data-toggle="tooltip" class="tooltips" data-original-title="Blind"><button id="direct_cmt_blind_li_<?php echo $item["comment_id"] ?>" class="btn-e btn-e-light-grey btn-e-xs btn-cmt-blind" data-cmt-id="<?php echo $item["comment_id"] ?>"><i class="fa fa-eye-slash"></i></button></li>
								<?php } elseif ($item["yc_blind"] == 'y') { ?>
								<li data-placement="top" data-toggle="tooltip" class="tooltips" data-original-title="Cancel"><button id="cancel_cmt_blind_li_<?php echo $item["comment_id"] ?>" class="btn-e btn-e-dark btn-e-xs btn-cmt-blind" data-cmt-id="<?php echo $item["comment_id"] ?>"><i class="fa fa-eye"></i></button></li>
								<?php } ?>
							<?php } ?>
						<?php } ?>
						<!--{* 댓글 신고 블라인드 끝 *}-->
...
$(function() {
	...
	$('body').on('click', '.cmt_yellow_card, .cancel_cmt_yellow_card', function () {
