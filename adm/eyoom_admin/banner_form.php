...
<div class="tbl_frm01 tbl_wrap">
	...
	<tr>
		<th scope="row"><label for="bn_img">배너이미지(자체 업로드)</label></th>
		<td colspan="3">
	...
	<tr>
		<th scope="row"><label for="bn_img_external">배너이미지(외부 이미지)</label></th>
		<td colspan="3">
			<?php
				if ($banner['bn_img'] && preg_match("/(http|https):/i", $banner['bn_img'])) {
					echo '<img src="'.$banner['bn_img'].'" alt="" height=80><br><br>';
					echo '<input type="text" name="bn_img_external" id="bn_img_external" class="frm_input" size="80" value="'.$banner['bn_img'].'">';
				} else {
					echo '<input type="text" name="bn_img_external" id="bn_img_external" class="frm_input" size="80" value="">';
				}
			?>
		</td>
	</tr>
...
function fbanner_submit(f)
{
	...
	if($("#bn_type > option:selected").val() == 'intra') {
		if(($("#bn_img").val() == '' && $("#del_bn_url_name").val() == '') && $("#bn_img_external").val() == '') {
